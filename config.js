// Настройки — клиент меняет только этот файл
// Настройки — клиент меняет только этот файл
const CONFIG = {
  pageTitle: "Авторськая практика від психолога",
  mainTitle: "Авторськая практика від психолога-гіпнотерапевта",
  highlight1: "ТЕХНІКА ШВИДКОГО ВІДНОВЛЕННЯ",
  subtitle: "15 ХВИЛИН, щоб позбутись стресу і напруги",
  newPrice: "149 грн",
  oldPrice: "2198 грн",
  highlight2: "Ціна однієї кави замість сессії з психологом",
  paymentLink: "https://psychologist-rumiantsevas-bot.tg.pulse.is",
  // background может быть относительным путём (content-images/background.jpg) или внешним URL
  background: "https://i.postimg.cc/VNYR1jcw/nm.jpg"
};

